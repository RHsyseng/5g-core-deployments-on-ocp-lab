...
The Topology Aware Lifecycle Manager (TALM) builds the remediation plan from the `ClusterGroupUpgrade` (CGU) custom resource for a group of clusters. You can define the following specifications in a ClusterGroupUpgrade CR. Note that this is a CGU CR that enforces an OpenShift release upgrade which was previously defined via a set of PolicyGenerator CRs (e.g. `core-upgrade*.yaml`) provided with the Core RDS.
...
WARNING: TALM does not only deal with OpenShift release upgrades, when we talk about upgrades in this section, we mean all kind of modifications to the managed clusters. Essentially, a CGU must be created every time we want to enforce a set of policies.
...
A ZTP wave is basically an annotation (`ran.openshift.io/ztp-deploy-wave`) included on each policy that will permit TALM to apply policies in an ordered manner. When using PolicyGenerator (the recommended approach), the wave is added to the `policyAnnotations` field of each policy entry in the PolicyGenerator CR. For example, the following excerpt from a {core-source-crs}[Core RDS] PolicyGenerator sets wave `"2"` for the subscriptions policy:
...
IMPORTANT: Detailed information on policy templates can be found in the following link:policygen-deepdive.html#policies-patching[PolicyGen deepdive] section.
