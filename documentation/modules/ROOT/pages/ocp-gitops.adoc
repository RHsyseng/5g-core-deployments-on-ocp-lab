...
* During fleet upgrades or phased rollouts, *different clusters in the same fleet are often on different OpenShift releases at the same time*—for example, cluster A still on the current release while cluster B has already been upgraded. The same pattern applies when *one or a few clusters run a pre-GA or candidate release* as a test bed while the rest of the fleet stays on the supported release. A directory layout (such as per-release folders for policy and RDS content) keeps each release's configuration distinct on one branch, and cluster labels bind each cluster to the release it should receive. A branch-per-release model duplicates that structure and complicates comparison and promotion between releases.
