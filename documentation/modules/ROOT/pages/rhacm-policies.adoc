...
Each `Policy` can contain one or multiple policy rules that will be evaluated against cluster(s) that they are bound to. Policy binding is determined by the RHACM placement APIs, which include the `PlacementBinding` and `Placement` CRs. 

WARNING: The older `PlacementRule` API is deprecated; use `Placement` instead from RHACM 2.7 and later.

Note that the PlacementBinding connects the Policy to a `Placement`, and the Placement defines where (on what clusters) the Policy should be active. View the following example specification of a Policy object:
...
The PlacementBinding CRD is used to bind the Policy with a `Placement`. Only a bound Policy is distributed to a managed cluster by the policy framework. View the following example specification of a PlacementBinding object:
...
placementRef:
  ...
  kind: Placement
  name: common-core-config-placement
subjects:
...
The `Placement` CRD is used to determine the target clusters to distribute policies to. A Placement selects clusters based on labels on the `ManagedCluster` CR on the hub cluster. Each target (spoke) cluster under management by RHACM will have a corresponding `ManagedCluster` CR. The placement controller writes a `PlacementDecision` listing the selected clusters; you do not define that object in Git.
