...
NOTE: These are absolute minimums to boot OpenShift. Where hyper-threading is enabled, 4 vCPU corresponds to 2 physical CPUs (4 hyper-threads), matching the Core RDS platform reservation described below. Telco Core production clusters running CNFs, DPDK-based UPF, or ODF storage require significantly higher resources. Sizing must be validated against your specific workload profile.
...
* **Web-based Assisted Installer** — provides a GUI wizard at https://console.redhat.com/openshift/assisted-installer/clusters[Red Hat OpenShift Cluster Manager] for installing individual clusters in connected environments. It does not scale to fleet management and is a poor fit for fully disconnected deployments, where Telco Core clusters typically operate.
